 ...
 * @brief   AT32 RTC subsystem low level driver header.
 *
 * @addtogroup RTC
 * @{
 */
 ...
 * @isr
 */
OSAL_IRQ_HANDLER(AT32_RTC1_HANDLER) {
  uint16_t flags;
...
void rtc_lld_set_prescaler(void) {
  ...
  rtc_acquire_access();
  RTC->PRLH = (uint16_t)((AT32_RTCCLK - 1) >> 16) & 0x000F;
  RTC->PRLL = (uint16_t)(((AT32_RTCCLK - 1))      & 0xFFFF);
  rtc_release_access();
...
void rtc_lld_init(void) {
  ...
  nvicEnableVector(AT32_RTC1_NUMBER, AT32_RTC_IRQ_PRIORITY);
}

/**
 * @brief   Set current time.
 * @note    Fractional part will be silently ignored. There is no possibility
 *          to change it on AT32F1xx platform.
...
void rtc_lld_get_time(RTCDriver *rtcp, RTCDateTime *timespec) {
  ...
  rtcAT32GetSecMsec(rtcp, &tv_sec, &tv_msec);
 ...
 * @api
 */
void rtcAT32GetSecMsec(RTCDriver *rtcp, uint32_t *tv_sec, uint32_t *tv_msec) {
  uint32_t time_frac;
  syssts_t sts;
  ...
  if (NULL != tv_msec)
    *tv_msec = (((uint32_t)AT32_RTCCLK - 1 - time_frac) * 1000) / AT32_RTCCLK;
}
 ...
 * @api
 */
void rtcAT32SetSec(RTCDriver *rtcp, uint32_t tv_sec) {
  syssts_t sts;
