...
void dmaInit(void) {
  ...
  for (i = 0; i < AT32_DMA_STREAMS; i++) {
    _at32_dma_streams[i].channel->CCR = AT32_DMA_CCR_RESET_VALUE;
...
const at32_dma_stream_t *dmaStreamAllocI(uint32_t id,
  ...
  for (i = startid; i <= endid; i++) {
    ...
    if ((dma.allocated_mask & mask) == 0U) {
...
#if (AT32_DMA_SUPPORTS_DMAMUX == TRUE) && defined(rccEnableDMAMUX)
      ...
      dmastp->channel->CCR = AT32_DMA_CCR_RESET_VALUE;
...
#if (AT32_DMA_SUPPORTS_DMAMUX == TRUE) && defined(rccDisableDMAMUX)
...
void dmaServeInterrupt(const at32_dma_stream_t *dmastp) {
  ...
  flags = (dmastp->dma->ISR >> dmastp->shift) & AT32_DMA_ISR_MASK;
