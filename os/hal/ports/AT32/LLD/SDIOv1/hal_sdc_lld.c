...
#define DMA_CHANNEL                                                         \
  AT32_DMA_GETCHANNEL(AT32_SDC_SDIO_DMA_STREAM,                           \
                       AT32_SDC_SDIO_DMA_CHN)
...
static bool sdc_lld_prepare_read_bytes(SDCDriver *sdcp,
  ...
  sdcp->sdio->DTIMER = AT32_SDC_READ_TIMEOUT;
  ...
  dmaStreamSetMode(sdcp->dma, sdcp->dmamode | AT32_DMA_CR_DIR_P2M);
  dmaStreamEnable(sdcp->dma);

  /* Setting up data transfer.*/
  sdcp->sdio->ICR   = AT32_SDIO_ICR_ALL_FLAGS;
...
#if (defined(AT32F4XX) || defined(AT32F2XX))
  /* Wait until DMA channel enabled to be sure that all data transferred.*/
  while (sdcp->dma->stream->CR & AT32_DMA_CR_EN)
    ;
...
void sdc_lld_start(SDCDriver *sdcp) {
  ...
  sdcp->dmamode = AT32_DMA_CR_CHSEL(DMA_CHANNEL) |
                  AT32_DMA_CR_PL(AT32_SDC_SDIO_DMA_PRIORITY) |
                  AT32_DMA_CR_PSIZE_WORD |
                  AT32_DMA_CR_MSIZE_WORD |
                  AT32_DMA_CR_MINC;

#if (defined(AT32F4XX) || defined(AT32F2XX))
  sdcp->dmamode |= AT32_DMA_CR_PFCTRL |
                   AT32_DMA_CR_PBURST_INCR4 |
                   AT32_DMA_CR_MBURST_INCR4;
#endif
...
#if (defined(AT32F4XX) || defined(AT32F2XX))
    dmaStreamSetFIFO(sdcp->dma, AT32_DMA_FCR_DMDIS | AT32_DMA_FCR_FTH_FULL);
#endif
    rccEnableSDIO(true);
  }
...
void sdc_lld_start_clk(SDCDriver *sdcp) {
  ...
  sdcp->sdio->CLKCR  = AT32_SDIO_DIV_LS;
...
#if AT32_SDC_SDIO_50MHZ
  if (SDC_CLK_50MHz == clk) {
    sdcp->sdio->CLKCR = (sdcp->sdio->CLKCR & 0xFFFFFF00U) | AT32_SDIO_DIV_HS
                                                          | SDIO_CLKCR_BYPASS;
  }
  else
    sdcp->sdio->CLKCR = (sdcp->sdio->CLKCR & 0xFFFFFF00U) | AT32_SDIO_DIV_HS;
...
bool sdc_lld_send_cmd_long_crc(SDCDriver *sdcp, uint8_t cmd, uint32_t arg,
  ...
  sdcp->sdio->ICR = sta & (SDIO_STA_CMDREND | SDIO_STA_CTIMEOUT |
                           SDIO_STA_CCRCFAIL);
  if ((sta & (AT32_SDIO_STA_ERROR_MASK)) != 0) {
    sdc_lld_collect_errors(sdcp, sta);
    return HAL_FAILED;
  }
...
bool sdc_lld_write_aligned(SDCDriver *sdcp, uint32_t startblk,
  ...
  sdcp->sdio->DTIMER = AT32_SDC_WRITE_TIMEOUT;
  ...
  dmaStreamSetTransactionSize(sdcp->dma,
                             (blocks * MMCSD_BLOCK_SIZE) / sizeof (uint32_t));
  dmaStreamSetMode(sdcp->dma, sdcp->dmamode | AT32_DMA_CR_DIR_M2P);
  dmaStreamEnable(sdcp->dma);
